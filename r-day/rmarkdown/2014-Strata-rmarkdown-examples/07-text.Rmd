...
## Collapse output chunks

```{r demo, collapse=TRUE, tidy.opts=list(keep.blank.line=FALSE)}
```
...
This is what you need when generating "raw" content in your output, e.g. tables.

```{r}
# there are many other packages for creating tables, e.g. the xtable package
knitr::kable(head(iris, 30))
```
